...
import { go, openWin, openLinks, openThirdLinks } from './utils/common'
...
if (ua.indexOf('apicloud') !== -1) {
  window.apiready = function() {
    ...
    Vue.mixin({
      mounted() {
        let name = this.$options.nameimport 'es6-promise/auto'
        import Vue from 'vue'
        import App from './App.vue'
        import router from './router'
        import store from './store'
        ...
        function exit_app() {
          api.addEventListener({ name: 'keyback' }, function(ret, err) {
            api.toast({ msg: '再按一次返回键退出' + api.appName, duration: 2000, location: 'bottom' })
