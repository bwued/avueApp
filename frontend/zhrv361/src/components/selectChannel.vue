    ...
    <ul class="list">
      <li v-for="(item,index) in list" :key="index" :class="{'item':true,'on':activeId===index}" @click="selectedChannel(item,index)">
        ...
        <div class="text">
          ...
          <div class="tip textEllipsis">
            提示：{{ item.intro }}
          </div>
        </div>
        <span class="detail-btn" @click="showDetailFun(item)">
          通道详情
          <i class="iconfont icon-gengduo" />
        </span>
      </li>
    </ul>
    <div class="add-btn" @click="$router.push('/addPlan?code='+selectedChannelItem.code)">
      确定
    </div>
    <van-popup v-model="showDetail" class="detail">
      <img class="img" :src="showDetailItem.logo_image&&showDetailItem.logo_image.url">
      <div class="name">
        {{ showDetailItem.name }}
      </div>
      <div class="dec-text">
        费率：{{ (showDetailItem.rate * 100).toFixed(2) }}%, {{ (showDetailItem.big_money_user_rate * 100).toFixed(2) }}% 
      </div>
      <div class="dec-text">
        交易时间：{{ showDetailItem.start_time }} - {{ showDetailItem.end_time }}
      </div>
      <div class="tip">
        <span v-html="showDetailItem.description" />
      </div>
      <div class="add-btn" @click="showDetail=false">
        确定
      </div>
    </van-popup>
  </div>
