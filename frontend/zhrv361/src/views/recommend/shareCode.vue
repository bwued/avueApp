...
export default {
  ...
  created() {
    ...
    this.$api.login.getQrcodeUrl(null).then(res => { // TODO 20200420 分享注册 接口调用 lhw
	  	if (res.data.data) {
        this.codeValue = res.data.data.qr_code_url
        // alert(this.codeValue)
        console.log('分享图片 二维码 (VUE_APP_SHARECODE_URL):', this.codeValue)
