<template>
  <div>
    ...
    <div class="con_body">
      ...
      <router-link v-show="false" to="/receivingAddress" class="line flex-row-between">
        <span>收货地址</span>
        <i class="iconfont icon-Path" />
      </router-link>
      <router-link to="/permanentAddress" class="line flex-row-between">
        <span>常住地址</span>
        <i class="iconfont icon-Path" />
      </router-link>
      ...
      <!-- <div v-show="checkFlag" class="line flex-row-between" @click="upgrade">
        <span>检查更新<span v-if="version !=banbenhao" class="z_dd" /></span>
        <i class="iconfont icon-Path" />
      </div>-->
    </div>
  ...
  methods: {
    ...
    upgrade1() {
      ...
      axios.post('http://120.79.74.34:6062/upgrade/version/get', params).then(res => {
        ...
        this.banbenhao = data.data.version
        console.log(this.banbenhao)
      }).catch(err => {
        console.log(err)
        Toast('服务异常')
      })
