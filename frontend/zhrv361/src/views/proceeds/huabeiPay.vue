<template>
<!-- 花呗TX 20200601 todo Ps: 不需选择信用卡  -->
  <div class="mgb100">
    <top-msg header-name="选择储蓄卡" />
    <div class="con_body">
      ...
      <div class="select_content">
        ...
        <div class="select_make" v-if="isHBFQ">
          <div class="form_cellBd clear">
            <span class="select-city-label">分期</span>
            <span class="color999 rt size30" type="text" @click="showPicker = true">
              {{ repay_type }}
              <van-icon name="arrow" />
            </span>
          </div>
        ...
        <div class="select_warp" v-if="false" style="dispaly:none;">
...
export default {
  ...
  data: function() {
    return {
      ...
      stagesNum: 6,
      columns: ['6期', '12期'],
      isHBFQ: this.$util.getQueryVariable('param_type') === 'HBFQ' ? true : false, // 是否显示花呗分期
      ...
      areaList: '',
      cityVal: '',
      checkDEBITFlag: false,
      checkCreatFlag: false,
      areaPicker: false,
  ...
  methods: {
    onConfirm(value) {
      ...
      if (this.repay_type == '6期') {
        this.stagesNum = 6
      }
      if (this.repay_type == '12期') {
        this.stagesNum = 12
      }
      this.showPicker = false
    },

    registjudgeNew(channelCode) { // todo 新版本
      const that = this
      that.$api.card.registjudgeNew(channelCode, that.cardId1, 'DEBIT').then(res => {
        console.log(res.data.data.state)
        const state = res.data.data.state
        if (state == 3) { //   3.未注册 4.已注册
          that.$api.card.regchannel(that.cardType1, that.cardId1, that.channelCode).then(res => {
            ...
            if (isSign == 't' && state == '000') {
              ...
              //                    H5 储蓄卡签约
              that.$router.push({ path: '/iframe', query: { title: '银行卡签约', url: html }})
            }
          }).catch(error => {
            console.log('储蓄卡注册失败')
            console.log(error)
    ...
    /* todo 信用卡检查和及签约 H5/api  ok */
    checkCreat(channelCode, cardId, cardType) {
      const that = this
      this.$api.card.txCheck(channelCode, cardId, cardType || 'CREDIT').then(res => {
        console.log(res.data.data.state)
        const signType = res.data.data.data.signType // signType = h5
        // 绑卡类型: 0. 无需绑卡 1.接口绑卡 2.H5绑卡, 3.第三方h5
        if (signType == 1) { //   1.未注册 2.已注册
          that.bindCardSF('CREDIT', cardId, channelCode, '信用卡签约') // 签约绑卡四方 = H5
        } else {
          //               Toast('信用卡检查已注册')
          this.checkCreatFlag = true
        }
      })
    },
    // todo 新储蓄卡注册判断
    checkDEBIT(channelCode) {
      const that = this
      that.$api.card.registjudgeNew(channelCode, that.cardId1, 'DEBIT').then(res => {
        console.log(res.data.data.state)
        const state = res.data.data.state
        if (state == 3) { //   3.未注册 4.已注册
          that.txCheckBindCard('DEBIT')
        } else {
          //                Toast('储蓄卡检查已注册')
          that.checkCreat(that.channelCode, that.creditCardMsg.id, 'CREDIT')
    ...
    payFun() {
      ...
      if (!that.debitCardMsg.id) {
        ...
        if (that.isHBFQ) {
          orderData.stages_num = that.stagesNum
        }
        ...
        that
          ...
          .then(() => {
            ...
            //              that.registjudge(that.channelCode) // ok todo 以前

            that.registjudgeNew(that.channelCode) // todo // 注册渠道信息判断 储蓄卡
            return false

            that.checkDEBIT(that.channelCode) // 注册渠道信息判断 储蓄卡 todo
            if (this.checkDEBITFlag && this.checkCreatFlag) {
              that.payment() // todo 暂时注解 下单接口
            }
    ...
    txCheckBindCard(cardType) { // todo 套现储蓄卡检查绑卡 毕 判断接口或h5
      const that = this
      that.$api.card.regchannel(cardType, that.cardId1, that.channelCode).then(res => {
        var isSign = res.data.data.isSign
        var state = res.data.data.state
        var html = res.data.data.html
        ...
        if (isSign == 't' && state == '000') {
          console.log('签约')
          that.checkCreat(that.channelCode, that.creditCardMsg.id, 'CREDIT') // todo 信用卡检查和及签约 H5/api
        } else {
          that.$router.push({ path: '/iframe', query: { title: '银行卡签约', url: html }})
        }
      }).catch(error => {
    ...
    // TODO 签约绑卡四方 = H5
    bindCardSF(cardType, cardId, channelCode, title) {
      const that = this
      that.$api.card.regchannel(cardType, cardId, channelCode).then(res => {
        var isSign = res.data.data.isSign
        var state = res.data.data.state
        var html = res.data.data.html

        if (isSign == 'f' && state == '000') {
          //                  window.location.href = html
          that.$router.push({ path: '/iframe', query: { title: title || '银行卡签约', url: html }})
        } else {
          console.warn('签约绑卡四方,,,t')
        }
      }).catch(error => {
        console.log(error)
        Toast('注册失败')
      ...
      that.$api.card
        .bbbindChannelCardtx(channelCode, cardId, cardType).then(res => {
          ...
          if (signature !== 2) { //   1.未签约 2.已签约
            that.$api.card.bindChannelCard(cardType, cardId, channelCode).then(res => { // 储蓄卡
              Toast.clear()
              const state = res.data.data.state
              ...
              if (state != '000') {
                ...
                if (platformType === 'sf') {
                  if (isSign === 't') {
                    that.payment() // 下单接口 // todo 暂时注解
                    return false
                  }
                  // TODO 定义 四方h5 储蓄卡绑卡签约 -啊兵 20200303
                  that.$router.push({ path: '/iframe', query: { title: '银行卡签约', url: _url }})
                  return false
                } else {
                  that.bindCardSF(that.cardType1, that.cardId1, that.channelCode) // TODO H5 绑卡
                }
