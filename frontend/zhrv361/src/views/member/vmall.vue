<template>
  <div>
    ...
    <!-- <top-msg header-name="申请礼包" /> -->
    <div class="con_body mgb110">
      <div class="bg_msg" style="display:block;">
      ...
      <div class="member_head">
        ...
        <div style="position:fixed; top:100px; left:80px; display:none;">
          <template v-if="userInfo">
            <h5
              v-if="userInfo.id_card_name"
              class="user-name textEllipsis"
        ...
        <div v-if="userInfo.rank_name === '平台合伙人'" class="z_percentage">
          <div class="div1">
            <p id="p1">
      ...
      <!-- 礼包类型隐藏 -->
      <div class="meal_warp bgf8f8f8" style="display:none;">
        <div class="meal_title flex-row-between">
          <span class="update-package">选择礼包类型</span>
        ...
        <!-- 微信支付取消，改为购买礼包，代付,子商户或运营后台 审核 -->
        <div class="update-pay-methods" style="display:none;">
    ...
    <van-dialog v-model="explainDialog" :show-confirm-button="showFalse" close-on-click-overlay>
      <div>
        ...
        <div class="explain_content">
          <p>成为礼包后可以获得平台更多的让利和服务，不同类型的礼包所有享受的费率和分润都不一样，级别越高，费率越低，分润越高，详情可咨询平台客服</p>
        </div>
        <div class="cen mb40">
    ...
    <!-- <embed src="http://www.baidu.com" width=200 height=200 /> -->
  </div>
</template>
  ...
  methods: {
    ...
    toggle(i) {
      ...
      if (
        ...
        console.log(this.feeUpgradeEnableFeeList[i].rank.rank_order + '::' + this.feeUpgradeEnableFeeList[i].rank.rank_name)
        this.rank_order = this.feeUpgradeEnableFeeList[i].rank.rank_order
        this.rank_name = this.feeUpgradeEnableFeeList[i].rank.rank_name
      }
	  console.log(this.feeUpgradeEnableFeeList[i].rank)
    },
    // todo 我的礼包 虚拟礼包 2020
    userUpgrade() {
      const that = this
      ...
      // }
	    const params = {
      // upgradeLevel: that.rank_order
        upgradeLevel: '0'
	  }
	  // alert(JSON.stringify(params))
      that.$api.member.userUpgrade(params).then(res => {
        console.log(res)
        if (res.data.code == '200000') {
          this.$dialog({
            ...
            // that.$router.push({ path: '/xiframe', query: { title: '智慧人商城', url: 'https://mall.guangzhouhuitong.cn/addons/yun_shop/?menu#/home?i=4' }})
		    that.$router.push({ path: '/hiframe', query: { title: '智慧人商城', refer: '/center', url: 'https://mall.guangzhouhuitong.cn/addons/yun_shop/?menu#/home?i=4' }})	
            // that.$router.push({ path: '/iframe', query: { title: '百度一下', url: 'http://www.baidu.com' }})
            // window.location.href = 'https://mall.guangzhouhuitong.cn/addons/yun_shop/?menu#/home?i=4'
          }).catch(() => {
    ...
    /* 点击升级*/
    upgradeMember(type) {
    //  Toast('温馨提示：恭喜您申请成功，系统将3分钟内自动为您改变服务礼包！')
      const that = this
      ...
      if (type === '1') {
        this.$dialog({
          title: '温馨提示',
          message: '您申请服务包成功后，系统将3分钟内自动为您改变服务礼包！',
          confirmButtonText: '确定',
          showCancelButton: true,
          cancelButtonText: '取消',
          cancelButtonColor: '#1989fa'
