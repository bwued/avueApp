<template>
  <div>
    ...
    <!-- 使用APICloud云打包 -->
    <div v-if="J_udge==='APICloud'" class="btn_warp">
      <button v-if="grade < feeUpgradeEnableFeeList.length" type="button" @click="y_openTypeSelect">
        确定购买
      </button>
    ...
    <!-- 使用原生离线打包 -->
    <div v-if="J_udge==='APIyun'" class="btn_warp">
    ...
    <!-- 选择支付方式 1是微信 2是快捷 -->
    <van-actionsheet
      v-model="y_showTypeSelect"
      :actions="actions"
      cancel-text="取消"
      @select="y_onTypeSelect"
      @cancel="y_nTypeCancel"
    />
    <van-actionsheet
...
export default {
  ...
  methods: {
    ...
    // APIcloud云打包
    y_openTypeSelect() {
      this.y_showTypeSelect = true // 云
    },
    /* 取消支付方式选择 */
    y_nTypeCancel() {
      this.y_showTypeSelect = false // 云
    },
    y_onTypeSelect(item) {
      this.y_showTypeSelect = false
      this.y_upgradeMember(item.id) // 调用接口方法
    },
    y_upgradeMember(type) {
      // return
      //  Toast('温馨提示：恭喜您购买成功，系统将3分钟内自动为您改变服务等级！')
      const that = this
      const msg = that.feeUpgradeEnableFeeList[that.currentSelect]
