  ...
  <div class="intelligence-page">
    ...
    <ul class="top-list">
      ...
      <li class="clear item" @click="showDateSelectFun">
        <span class="lt text">选择还款日期</span>
...
export default {
  ...
  methods: {
    ...
    getDefaultAddress() {
      const that = this
      that.$api.center.getDefaultAddress().then(res => {
        console.warn('获取默认地址.....')
        if (res.data.data && res.data.data.address_info && res.data.data.address_info.address_tag) {
          console.log(res.data.data.address_info.address_tag)
          that.selectedAreaList = JSON.parse(res.data.data.address_info.address_tag)
        } else {
          if (that.cityRecord) {
            that.selectedAreaList = that.cityRecord
          }
        }
      })
    },
    ...
    onConfirm(value) {
      ...
      if (this.repay_type == '1消1还') {
        this.reapyType = 1
      }
      if (this.repay_type == '2消1还') {
        this.reapyType = 2
      }
      if (this.repay_type == '3消1还') {
        this.reapyType = 3
      }
      this.getRepayCount()
    ...
    changeNum() {
      if (this.repay_sum_amount) {
        //            if(!(/(^[1-9]\d*$)/.test(this.repay_sum_amount))){
        //                this.$dialog({
        //                    title: '温馨提示',
        //                    message: '账单总金额需取整数，请重新输入账单总金额',
    ...
    closeTime() {
      ...
      // console.warn(this.date.replace(/-/g,"/"))
      // var stringTime = this.date + " " + s
      var stringTime = this.date.replace(/-/g, '/') + ' ' + s
    ...
    getRepayCount() {
      this.$api.plan.getRepayCount({ repay_type: this.reapyType }).then(res => {
    ...
    async toAddBill() {
      var data = {
        bank_card_id: this.bankMsg.id, // 银行卡id
        // channel_code: this.channel_code,
        city_code: this.selectedAreaList.region, // 行政编号
        repay_days: this.repay_days, // 还款日期
        repay_sum_amount: this.repay_sum_amount * 100, // 还款总金额
        //        repay_sum_count: this.value
        // repay_count_per_day: this.value, // 每天还款笔数
        // repay_type: this.reapyType,
        principal_amount: this.principal_amount * 100 // 信用卡预留金额
      }
      ...
      this.$api.plan.genIntelligentV3(data).then(res => {
        ...
        if (res.data.code === '200000') {
          ...
          const delay_msg = res.data.data.delay_msg
          //          const delay_msg = "延迟计划测试"
          if (delay_msg) {
            this.$dialog({
              title: '温馨提示',
              message: delay_msg,
    ...
    judeRange(useDay, repayDay) {
      ...
      if (use < repay) {
        ...
        if (nowDate >= repay) {
          ...
          this.minDate = new Date(now.setMonth(nowMonth + 1, use)) // todo  当天日期可选
        ...
        if (nowDate <= use) {
          ...
          this.minDate = new Date(now.setMonth(nowMonth, use))
        ...
        if (nowDate > use && nowDate < repay) {
          ...
          this.minDate = new Date(now.setMonth(nowMonth, nowDate))
          this.maxDate = new Date(now.setMonth(nowMonth, repay - 1))
        }
      } else if (use > repay) {
        ...
        if (nowDate >= repayDay && nowDate >= use) {
          //          this.minDate = new Date(now.setMonth(nowMonth, nowDate + 1))
          this.minDate = new Date(now.setMonth(nowMonth, nowDate))
          this.maxDate = new Date(now.setMonth(nowMonth + 1, repay - 1))
        }
