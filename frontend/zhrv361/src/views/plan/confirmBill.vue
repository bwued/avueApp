  ...
  <van-popup v-model="isShowConfirmBill" class="confirm-bill-page">
    ...
    <ul class="list">
      ...
      <li class="item clear">
        ...
        <span class="rt text price">{{ checkBillList&&checkBillList.order_total_money }}元</span>
      </li>
      <li class="item clear">
      ...
      <li class="item clear">
        <span class="lt text">{{refundName}}:</span>
        <span class="rt text price">{{ checkBillList&&checkBillList.fee_refund_money }}元</span>
      </li>
      <li class="item clear">
...
export default {
  ...
  props: ['cRate', 'showConfirmBill', 'toHideConfirmBill', 'toShowConfirmBill', 'fromIntelligencePage', 'checkBillList', 'minDate', 'maxDate', 'confirmMsgList', 'bankCardId','listData'],
  data() {
    return {
      isShowConfirmBill: false,
      refundName:'智慧金' // todo 智慧金 接口获取
    }
  },
  computed: {
    //   user_rate() {
  ...
  methods: {
    ...
    confirm() {
      ...
      this.$api.plan.addRepayPlan({ bank_card_id: this.bankCardId, trade_no: this.fromIntelligencePage ? this.checkBillList.json_trade_no : this.confirmMsgList.plan_info.json_trade_no, modified_bill_list:this.listData }).then(res => {
