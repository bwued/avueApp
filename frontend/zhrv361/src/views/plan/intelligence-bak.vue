  ...
  <div class="intelligence-page">
    ...
    <ul class="top-list">
      ...
      <li class="clear item" @click="showDateSelectFun">
        <span class="lt text">选择到期日期</span>
      ...
      <div class="situation">
        ...
        <div class="item">
          <div class="clear title">
            ...
            <span class="rt text">共{{ billMsg.useNum+billMsg.repayNum }}笔</span>
          </div>
          <div class="clear dec">
            <span class="lt text">还款总金额：
              <span class="price">{{ billMsg.repayPrice }}元</span>
            </span>
        ...
        <li v-for="(item,index) in billList" :key="index" class="item">
          <div class="title clear">
            <span class="lt text">
              ...
              还款¥{{ item.plan_bill_info&&item.plan_bill_info.amount }}
            </span>
  ...
  methods: {
        ...
        .bbbindChannelCard(channelCode, cardId, cardType).then(res => {
          ...
          if (signature != 2) { //   1.未签约 2.已签约
            ...
            if (signType == 3) { // 汇卡通道 信用卡未签约
              ...
              // that.getCodeFun(cardId, channelCode)
            }
          } else {
    ...
    async toAddBill() {
      ...
      if (this.max_count === 0 && this.min_count === 0) {
        ...
        // this.$toast('最大还款笔数不大于' + this.max_count + '笔')
        // return false
      } else if (data.repay_sum_count < this.min_count) {
        this.$toast('最小还款笔数不小于' + this.min_count + '笔')
        return false
      }
